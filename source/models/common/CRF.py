...
class CRF(nn.Module):
    ...
    def _forward_backward_algorithm(
        ...
        for i in range(1, seq_length):
            ...
            # shape: (batch_size, num_tags)
            score = torch.where(mask[i].unsqueeze(1).type(torch.bool), next_score, score)
            scores[i, :, :] = score
    ...
    def _viterbi_decode(
        ...
        return torch.where(mask.type(torch.bool), best_tags_arr, oor_tag).transpose(0, 1)
    ...
    def _viterbi_decode_nbest(
        ...
        for i in range(1, seq_length):
            ...
            score = torch.where(mask[i].unsqueeze(-1).unsqueeze(-1).type(torch.bool), next_score, score)
            indices = torch.where(mask[i].unsqueeze(-1).unsqueeze(-1).type(torch.bool), indices, oor_idx)
        ...
        return torch.where(mask.unsqueeze(-1).type(torch.bool), best_tags_arr, oor_tag).permute(2, 1, 0)
...
def viterbi_decode_inner_loop1(score, history_idx, emissions, transitions, mask, oor_idx):  # noqa
    ...
    for i in range(1, seq_length):
        ...
        # shape: (batch_size, num_tags)
        score = torch.where(mask[i].unsqueeze(-1).type(torch.bool), next_score, score)
        indices = torch.where(mask[i].unsqueeze(-1).type(torch.bool), indices, oor_idx)
        history_idx[i - 1] = indices
    return score
...
class PartialCRF(CRF):
    ...
    def __init__(self, num_tags: int, batch_first: bool = False) -> None:
        ...
        self.UNLABELED_INDEX = -100
        self.IMPOSSIBLE_SCORE = -10000
